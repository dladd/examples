...
def solveFemWithParametersNoInit(sobelovParameters):
    '''Solve Finite element fit with specified values of tau and kappa'''
    tau=abs(sobelovParameters[0])
    kappa=abs(sobelovParameters[1])
    # Set kappa and tau - Sobelov smoothing parameters
...
parser.add_option("-r","--resolution", action="store", type="int", dest="diameterResolution",
                   help='number of data points across the mesh diameter', default=10)
...
parser.add_option("-t", "--tetrahedral", default=True, action="store_true", help="flag whether tetrahedral mesh. default=False (hexahedral mesh)")
parser.add_option("-o", "--optimise", default=False, action="store_true", help="flag whether to optimise parameters. default=False")
parser.add_option("-i", "--fitIdw", default=True, action="store_true", help="flag to turn off IDW fit. default=True")
parser.add_option("-f", "--fitFem", default=False, action="store_true", help="flag to turn off FEM fit. default=True")
parser.add_option("-z", "--addZeroLayer", default=True, action="store_true", help="flag whether to add zero layer to IDW fit. default=True")
...
parser.add_option("-l", "--edgelength", action="store", type="int", help="edgelength", dest="edgelength")
...
meshDir = './input/refine/cylinder/'
meshName = 'cylinder' + str(edgelength)
fieldmlInput = meshDir + meshName + '.xml'
#fieldmlInput = inputDir + 'cylinder400.xml'
...
dataResolutionIncrement[axialComponent] = length/float(dataResolution[axialComponent])
...
if (addZeroLayer):
    ...
    for i in range(3):
        if i != axialComponent:
            dataInit[i] = init[i] - (float(additionalPoints)/2.0)*inc[i]
            dataResolution[i] += additionalPoints
            dataRadius += inc[i]**2.0
    dataRadius = math.sqrt(dataRadius)
    ...
    for z in range(dataResolution[2]):
        ...
        for y in range(dataResolution[1]):
            ...
            for x in range(dataResolution[0]):
                ...
                if (linalg.norm(radialPosition) <= radius + dataRadius + zeroTolerance):
#                if (linalg.norm(radialPosition) <= radius + linalg.norm(dataResolutionIncrement) + zeroTolerance):
...
#                if (linalg.norm(radialPosition) <= radius + linalg.norm(radialPosition) + zeroTolerance):
    #                print("Data Point position: " + str([dataPointX,dataPointY,dataPointZ]))
    dataResolution = [i - 10 for i in dataResolution]
else:
...
for point in range(numberOfDataPoints):
    ...
    test = parabolic([0.0,0.0,0.0],radius,uMean,axialComponent)
    #print('test '+str(test))
...
if (addNoise):
    ...
    velNorms = numpy.apply_along_axis(numpy.linalg.norm, 1, velocityData)    
    signalMean = numpy.mean(velNorms)
    print("signal mean: " + str(signalMean))
    #signalMean = numpy.mean(velocityData[:,axialComponent])
    #signalMean = numpy.mean(velocityData)
    SDsignal = abs(signalMean/SNR)
    print('std deviation of signal: '+str(SDsignal))
    for component in range(3):
        noise[:,component] = numpy.random.normal(0.0,SDsignal,numberOfDataPoints)
    noiseNorms = numpy.apply_along_axis(numpy.linalg.norm, 1, noise)    
    RMSE_data = numpy.sqrt(numpy.mean(noiseNorms**2))
    print('RMSE data points: ' + str(RMSE_data))
    ...
    for dataPoint in range(numberOfDataPoints):
        ...
        #    velocityData[dataPoint,:] += noise[dataPoint,:]
    outputNoiseRMSEFile = 'output/' + meshName + '/noise/SNR' + str(int(SNR)) +'/dataPointsRMSE_CDR' + str(diameterResolution) + '.txt'
    print('Writing data error to file: ' + outputNoiseRMSEFile)
    try:
        with open(outputNoiseRMSEFile, 'a') as file:    
            file.write(str(diameterResolution) + ', ' + str(RMSE_data) + ', \n')            
    except IOError:
        print ('Could not open RMSE log file: ' + outputNoiseRMSEFile)

    # print('Checking SNR: ')
    # anaMag = numpy.apply_along_axis(numpy.linalg.norm, 1,analyticVelocityDataPoints)
    # meanAnaMag = numpy.mean(anaMag)
    # velMag = numpy.apply_along_axis(numpy.linalg.norm, 1, velocityData)    
    # meanVelMag = numpy.mean(velMag)
    # stdVelMag = numpy.std(velMag)
    # checkSnr = meanVelMag/stdVelMag
    # print('  Magnitude: ' + str(velMag))
    # print('  Mean Magnitude: ' + str(meanVelMag))
    # print('  Mean Analytic Magnitude: ' + str(meanAnaMag))
    # print('  StdDev Magnitude: ' + str(stdVelMag))
    # print('  -----------------------')
    # print('  SNR: ' + str(checkSnr))
...
for node in xrange(numberOfNodes):
    ...
    if (nodeDomain == computationalNodeNumber):
        ...
        meshPosition[axialComponent] = 0.0
        radialPosition = numpy.linalg.norm(meshPosition)
        if abs(radialPosition - radius) < wallTolerance:
            wallNodes.append(nodeNumber)
...
if (fitFem):
    if (optimise):
        ...
        tau = tauMin
        kappa = kappaMin
        resultFem = optimize.fmin(solveFemWithParameters,[tau,kappa], xtol=0.00001)
        ...
        for iteration in range(fefIterations):
            ...
            fefVelRMSE = solveFemWithParametersNoInit([tau,kappa])            
            timeFemItStop = time.time()        
            timeIt = timeFemItStop - timeFemItStart
            print('Iteration Time: ' + str(timeIt) + '\n\n')
...
outputDirectory = 'output/meshRefine/'
...
outputDirectory = outputDirectory + 'idw/'
error = idwVelRMSE
# Create a results directory if needed
try:
    os.makedirs(outputDirectory)
...
wallNodeFile = outputDirectory+meshName+'wallNodes.dat'
print('Writing to wall node file: ' + wallNodeFile)
try:
    with open(wallNodeFile, 'w') as file:
        file.write(str(numberOfWallNodes) + ', \n')            
        line = str(wallNodes)
        line = line.replace('[','')
        line = line.replace(']','')
        file.write(line + ', \n')            
