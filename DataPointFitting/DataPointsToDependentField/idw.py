...
def idwFit(p,vicinityFactor,sourceLocations,sourceVectors,
           targetLocations,targetVectors,targetNodeList,wallNodes,dataResolutionIncrement):
    '''Solve IDW'''
    ...
    dataList = [[] for i in range(numberOfTargetPoints+1)]
    sumWeights = numpy.zeros((numberOfTargetPoints+1))

    timeIdwStart = time.time()        

    weight = numpy.zeros((numberOfTargetPoints+1,((vicinityFactor*2)**3 + numberOfWallNodes*3)))
    ...
    for node in targetNodeList:
        ...
        if(nodeNumberCmiss not in wallNodes[:]):
            ...
            if(fastSearch):
                minX = bisect.bisect_left(sortedData[:,0],meshPosition[0]-vicinityFactor*dataResolutionIncrement[0])
                maxX = bisect.bisect_right(sortedData[:,0],meshPosition[0]+vicinityFactor*dataResolutionIncrement[0])
                numberOfCandidates = maxX - minX
                dataPoint = -1
                fittingPoint = 0
                for candidate in xrange(numberOfCandidates):
                    if(abs(sortedData[minX+candidate,0] - meshPosition[0]) < vicinityFactor*dataResolutionIncrement[0] and
                       abs(sortedData[minX+candidate,1] - meshPosition[1]) < vicinityFactor*dataResolutionIncrement[1] and
                       abs(sortedData[minX+candidate,2] - meshPosition[2]) < vicinityFactor*dataResolutionIncrement[2]):
                ...
                if dataPoint == -1:
                    ...
                    print("Tolerance: " + str(vicinityFactor*dataResolutionIncrement[0]) + "\n")
                ...
                for dataPoint in xrange(numberOfSourcePoints):
                    ...
                    if(abs(sourceLocations[dataPoint,0] - meshPosition[0]) < vicinityFactor*dataResolutionIncrement[0] and
                       abs(sourceLocations[dataPoint,1] - meshPosition[1]) < vicinityFactor*dataResolutionIncrement[1] and
                       abs(sourceLocations[dataPoint,2] - meshPosition[2]) < vicinityFactor*dataResolutionIncrement[2]):
