...
absoluteCouplingTolerance3D1D = 1.0E-6
relativeCouplingTolerance3D1D = 1.0E-3
startTime = 0.0
stopTime = 500.00 + 1.0E-6
timeIncrement = 1.0
outputFrequency = 1
...
# 3D parameters
#-------------------------------------------
numberOfElements = 540 #3840
quadraticMesh = True
outletArteries = ['LIA','RIA']
analyticInflow = True
inletValue = 0.5 #500.0#0.001#1.0
normalInlet3D = numpy.zeros((3))
normalOutlets3D = numpy.zeros((2,3))
if numberOfElements == 3840:
    couplingNodes3D = [4210,4468]
    inletNode3D = 31
    inputDir = "./input/iliac3D/3840Elem/normal/"
    meshName = "iliac3840"
...
for couplingNode in couplingNodes1D:
    ...
    for element in range(branch*numberOfElements1D/2+1,(branch+1)*numberOfElements1D/2+1):
        ...
        #print("- Rank "+str(computationalNodeNumber)+" Element "+ str(element)+ " nodes: " + str(elementNodes))
        nodeIdx+=2
    branch+=1
# Set up dummy nodes
if numberOfComputationalNodes > 1:
...
for element in range(1,numberOfElements1D+1):
    checkNodes = meshElements.NodesGet(element,3)
    #print("-- Rank "+str(computationalNodeNumber)+" checking element "+ str(element)+ ". Found nodes: " + str(checkNodes))
mesh1D.CreateFinish() 
...
for nodeNumber3D in couplingNodes3D:
    ...
    for element in range(1,numberOfElements1D/2+1):
        elementNumber = branch*numberOfElements1D/2+element
        #print("Rank " + str(computationalNodeNumber) + " element number " + str(elementNumber) + " on " + str(nodeDomain))
        decomposition1D.ElementDomainSet(elementNumber,nodeDomain)
    branch+=1
# Dummy elements
if numberOfComputationalNodes > 1:
    for cnode in range(numberOfComputationalNodes):
...
# Set node positions
lengthIncrement=length1D/(float(numberOfNodes1D)/2.-1.)
for branch in range(2):
    ...
    for nodeNumber1D in range((numberOfNodes1D/2)*branch+1,(numberOfNodes1D/2)*(branch+1)+1):
        #print("  Rank "+str(computationalNodeNumber)+ " node "+str(nodeNumber1D))
        nodeDomain = decomposition1D.NodeDomainGet(nodeNumber1D,meshComponentNumber)
        if (nodeDomain == computationalNodeNumber):
            #print(" == Rank "+str(computationalNodeNumber)+ " node "+str(nodeNumber1D) + " on " +str(nodeDomain))
            for dimension in range(numberOfDimensions):
                value = firstNodeLocation[dimension] + normalOutlets3D[branch,dimension]*float(branchNode)*lengthIncrement
            ...
            continue
            #print("  Skip:   Rank "+str(computationalNodeNumber)+ " node "+str(nodeNumber1D) + " domain "+str(nodeDomain))
        branchNode+=1
...
if numberOfComputationalNodes > 1: 
    for dummyNodeNumber in range(numberOfNodes1D+1,numberOfNodes1D+2+numberOfComputationalNodes*2):
        ...
        if (nodeDomain == computationalNodeNumber):
            #print(" ++ Rank "+str(computationalNodeNumber)+ " dummy node "+str(dummyNodeNumber) + " on " +str(nodeDomain))
...
nonlinearSolver3D.newtonRelativeTolerance = 1.0E-10
...
for nodeNumber in inletNodes3D:
    nodeDomain=decomposition3D.NodeDomainGet(nodeNumber,meshComponent3DVelocity)
    if (nodeDomain == computationalNodeNumber):
        if boundaryType == CMISS.BoundaryConditionsTypes.FIXED_INLET:
            if nodeNumber not in wallNodes3D:
...
linearSolver3D.MumpsSetIcntl(14,100)
...
for nodeIdx in couplingNodes1D:
    nodeDomain = decomposition1D.NodeDomainGet(nodeIdx,meshComponentNumber)
    if (nodeDomain == computationalNodeNumber):
        print("Setting coupling flow BC for characteristic boundary node " + str(nodeIdx))
...
for nodeIdx in outletNodes1D:
    nodeDomain = decomposition1D.NodeDomainGet(nodeIdx,meshComponentNumber)
    if (nodeDomain == computationalNodeNumber):
        print("Setting nonreflecting BC for characteristic boundary node " + str(nodeIdx))
        boundaryConditions1DC.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
                                      1,1,nodeIdx,2,CMISS.BoundaryConditionsTypes.FIXED_NONREFLECTING,A0)
# Set dummy nodes to fixed 0
#for dummyNodeNumber in range(numberOfNodes1D+1,numberOfNodes1D+1+numberOfComputationalNodes):
#for dummyNodeNumber in range(numberOfNodes1D+1,numberOfNodes1D+2+numberOfComputationalNodes*2):
if numberOfComputationalNodes > 1:
    ...
    if (nodeDomain == computationalNodeNumber):
        ...
        boundaryConditions1DC.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
                                      1,1,dummyNodeNumber,2,CMISS.BoundaryConditionsTypes.FIXED,A0)
...
for nodeIdx in couplingNodes1D:
    nodeDomain = decomposition1D.NodeDomainGet(nodeIdx,meshComponentNumber)
    if (nodeDomain == computationalNodeNumber):
        print("Setting coupling flow BC for N-S boundary node " + str(nodeIdx))
...
for nodeIdx in outletNodes1D:
    nodeDomain = decomposition1D.NodeDomainGet(nodeIdx,meshComponentNumber)
    if (nodeDomain == computationalNodeNumber):
        print("Setting nonreflecting BC for N-S boundary node " + str(nodeIdx))
        boundaryConditions1DNS.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
                                       1,1,nodeIdx,2,CMISS.BoundaryConditionsTypes.FIXED_NONREFLECTING,A0)
...
if numberOfComputationalNodes > 1:
    ...
    # nodeDomain = decomposition1D.NodeDomainGet(dummyNodeNumber,meshComponentNumber)
    # if (nodeDomain == computationalNodeNumber):
    #     print("Setting dummy BCs for Navier-Stokes boundary node " + str(dummyNodeNumber))
    #     boundaryConditions1DNS.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
    #                                   1,1,dummyNodeNumber,1,CMISS.BoundaryConditionsTypes.FIXED,0.0)
    #     boundaryConditions1DNS.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
    #                                    1,1,dummyNodeNumber,2,CMISS.BoundaryConditionsTypes.FIXED,A0)
...
outputDirectory = "./output/Iliac_Dt" + str(round(timeIncrement/Ts,5)) + meshName + "/"
try:
