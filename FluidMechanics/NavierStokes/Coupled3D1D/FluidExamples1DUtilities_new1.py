...
def CsvNodeReader2(filename,inputNodeNumbers,branchNodeNumbers,coupledNodeNumbers,nodeCoordinates,branchNodeElements):
    ''' 1D Navier-Stokes problems are currently described using csv files- this routine reads in nodal data and returns it to the user.
    '''
    # Read the node file
    try:
        with open(filename,'rb') as csvfile:
            ...
            for row in reader:
                if (rownum == -1):
                    ...
                    if row[4] == 'inlet':
                        inputNodeNumbers.append(row[0])
                        inputArteryNames.append(row[5])
                    elif row[4] == 'terminal':                    
                        terminalNodeNumbers.append(row[0])
                        terminalArteryNames.append(row[5])                    
                    elif int(row[4]) != 0 and row[4] != '':
                        branchNodeNumbers.append(row[0])
                        thisNodeElements = []
                        for element in range(4,len(row)+1):
                            thisNodeElements.append(row[element])
        ...
        with open(filename,'rb') as csvfile:
            ...
            for row in reader:
                if (rownum == 0):
                    ...
                    elementNodeNumbers.append([int(row[1]),int(row[2]),int(row[3])])
        ...
        with open(filename,'rb') as csvfile:
            ...
            for row in reader:
                if (rownum == 0):
                    ...
                    nodeE=[]
                    nodeH=[]
                    for version in len(row)/3:
                        if row[version*3+1] == '':
                            break
                        else:
                            nodeA0.append(row[1+version*3])
                            nodeE.append(row[2+version*3])
                            nodeH.append(row[3+version*3])
                    A0.append(nodeA0)
                    E.append(nodeE)
                    H.append(nodeH)
                rownum+=1
