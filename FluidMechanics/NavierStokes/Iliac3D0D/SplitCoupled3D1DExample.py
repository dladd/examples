...
lengthIncrement = length1D/(numberOfNodes1D/2-1)
for node in range(numberOfNodes1D/2):
    nodeNumber = node+1
...
for branch in range(2):
    independentField1D.ParameterSetUpdateNodeDP(CMISS.FieldVariableTypes.U,CMISS.FieldParameterSetTypes.VALUES,
                                                versionIdx,1,couplingNodes1D[branch],2,-1.0)
# Outlet
versionIdx = 1
for branch in range(2):
    independentField1D.ParameterSetUpdateNodeDP(CMISS.FieldVariableTypes.U,CMISS.FieldParameterSetTypes.VALUES,
                                            versionIdx,1,outletNodes1D[branch],1,1.0)
# Finish the parameter update
...
for element in range(numberOfOutletElements3D):
    ...
    if (elementDomain == computationalNodeNumber):
        ...
        if element==0:
            equationsSetField3D.ParameterSetUpdateElementDP(CMISS.FieldVariableTypes.V,CMISS.FieldParameterSetTypes.VALUES,
                                                            elementNumber,11,couplingNodes1D[1])
        else:
            equationsSetField3D.ParameterSetUpdateElementDP(CMISS.FieldVariableTypes.V,CMISS.FieldParameterSetTypes.VALUES,
...
for branch in range(2):
    ...
    boundaryConditions1DC.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
                                   1,1,couplingNodes1D[branch],1,CMISS.BoundaryConditionsTypes.COUPLING_FLOW,Q0)
    # outlet boundary: nonreflecting
    boundaryConditions1DC.SetNode(dependentField1DNS,CMISS.FieldVariableTypes.U,
                                  1,1,outletNodes1D[branch],2,CMISS.BoundaryConditionsTypes.FIXED_NONREFLECTING,A0)
