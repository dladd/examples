...
#Right Iliac artery contains the stenosis and that is boundary 4
...
if (RCRBoundaries):
    ...
    modelFile = modelDirectory + 'CR.cellml'
    print('reading model: ' + modelFile)
    CellMLModel[0] = CellML.ModelImport(modelFile)
    # known (to OpenCMISS) variables
    CellML.VariableSetAsKnown(CellMLModel[0],"Circuit/Qi")
    # to get from the CellML side 
    #CellML.VariableSetAsWanted(CellMLModel[0],"Circuit/Pi")
    #modelFile = modelDirectory + 'RightIliac.cellml'
    modelFile = modelDirectory + 'CR.cellml'
    print('reading model: ' + modelFile)
    CellMLModel[1] = CellML.ModelImport(modelFile)
    # known (to OpenCMISS) variables
    CellML.VariableSetAsKnown(CellMLModel[1],"Circuit/Qi")
    # to get from the CellML side 
    #CellML.VariableSetAsWanted(CellMLModel[1],"Circuit/Pi")
    CellML.CreateFinish()
...
#Integrated flux i.e., outputflow is store in the equations set field!
                ...
                CellML.CreateFieldToCellMLMap(equationsSetField3D,CMISS.FieldVariableTypes.U,1,
                                              ...
                                              "Circuit/Qi",CMISS.FieldParameterSetTypes.VALUES)
                # Map the returned pressure values from CellML --> CMISS
                # pCellML is component 1 of the Dependent field U variable
                CellML.CreateCellMLToFieldMap(CellMLModel[outletIdx],"Circuit/Pi",CMISS.FieldParameterSetTypes.VALUES,
                                              dependentField3D,CMISS.FieldVariableTypes.U,4,CMISS.FieldParameterSetTypes.PRESSURE_VALUES)

                # Map the returned pressure values from CellML --> CMISS
                # pCellML is component 1 of the Dependent field U variable
                CellML.CreateCellMLToFieldMap(CellMLModel[outletIdx],"Circuit/Qi",CMISS.FieldParameterSetTypes.VALUES,
    ...
    CellMLStateField.ParameterSetCreate(CMISS.FieldVariableTypes.U,CMISS.FieldParameterSetTypes.PREVIOUS_VALUES)
