...
nodeFile = "./input/" + meshName + "/" + meshName + "D.C"
...
if readDataFromExnode:
    field = fieldInfo()
    filename = path + 'Wo' + woNums[0] + 'Dt' + str(cmfeTimeIncrements[0]) + meshName + '/TimeStep_00000.part0.exnode'
    print(filename)
    try:
        with open(filename):
        ...
        for cmfeTimeIncrement in cmfeTimeIncrements:
            ...
            for timestep in range(numberOfTimesteps[t]):
                ...
                for proc in range(numberOfProcessors):
                    filename = path + 'Wo' + wo + 'Dt' + str(cmfeTimeIncrement) +  meshName + '/TimeStep_' + str(timestep*cmfeOutputFrequencies[t]).zfill(5) + '.part' + str(proc) +'.exnode'
                    print(filename)
            ...
            filename = path + 'Wo' + wo + 'Dt' + str(cmfeTimeIncrement) +  meshName + '/Womersley_DT' + str(int(round(period/cmfeTimeIncrements[t],-1))) + 'H12N' + str(numberOfProcessors) +'.out'
...
if plotTime:    
    ...
    for timestep in timesteps:
        ...
        ana, num = plt.plot(x,analytic,lineColours[tIndex],x, numeric,'go')
        tIndex+=1
